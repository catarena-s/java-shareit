...
class ItemDtoJsonTest {
    ...
    void testUserDto_withBookingAndComment() {
        ...
        List<CommentDtoResponse> comments = List.of(
                CommentDtoResponse.builder().id(1L).itemId(1L).text("Comment 1").authorName("user1").created(currentTime.minusDays(2)).build(),
                CommentDtoResponse.builder().id(2L).itemId(5L).text("Comment 2").authorName("user2").created(currentTime.minusDays(6)).build()
        );
        ItemDtoResponse itemDto = ItemDtoResponse.builder()
                .id(1L)
                .name("Item name")
